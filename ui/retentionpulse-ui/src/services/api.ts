...
export type UploadResult = {
  ...
  failed_rows: number;
  predictions: Array<Prediction & { row: number }>;
...
export const api = {
  health: () => request<{ status: string; customer_count: number }>("/health"),
  ...
  explain: (data: CustomerData) =>
    request<Explanation>("/explain?top_k=5", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(data),
    }),
  upload: (csv: string) =>
    request<UploadResult>("/predict/upload", {
      method: "POST",
      headers: { "Content-Type": "text/csv" },
      body: csv,
    }),
  templateUrl: `${API_BASE}/csv-template`,
};
